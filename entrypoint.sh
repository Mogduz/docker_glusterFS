#!/usr/bin/env bash
# --- patched helpers: begin ---
pick_primary_ipv4(){
  ip -4 -o addr show scope global 2>/dev/null | awk '{print $4}' | cut -d/ -f1 | head -n1
}
ensure_hosts_mapping(){
  local cip="$(pick_primary_ipv4 || true)"
  if [[ -n "$cip" ]]; then
...
resolve_brick_host(){
  local cand=""
  if [[ -n "${BRICK_HOST:-}" ]]; then cand="${BRICK_HOST}"; fi
  if [[ -z "$cand" && -n "${HOSTNAME_GLUSTER:-}" ]]; then cand="${HOSTNAME_GLUSTER}"; fi
  if [[ -z "$cand" ]]; then cand="$(hostname -s)"; fi
  local prim="$(pick_primary_ipv4 || true)"
  if [[ -n "$prim" && "$cand" == "$prim" ]]; then
    cand="$(hostname -s)"
  fi
  if [[ -z "$cand" || "$cand" == "127.0.0.1" || "$cand" == "localhost" ]]; then
    echo "bad BRICK_HOST candidate: '$cand'" >&2
    return 1
  fi
  echo "$cand"
}
...
ensure_glusterd_vol
  if [[ -f /etc/glusterfs/glusterd.vol ]]; then
    if grep -q "option base-port" /etc/glusterfs/glusterd.vol; then
      sed -i 's/^\s*option\s\+base-port\s\+.*/        option base-port 49152/' /etc/glusterfs/glusterd.vol
    else
      sed -i '/type mgmt\/glusterd/a\        option base-port 49152' /etc/glusterfs/glusterd.vol
    fi
  else
