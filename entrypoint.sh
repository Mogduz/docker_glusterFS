...
# Ensure glusterd configuration matches env (address family, ports)
ensure_glusterd_vol(){
...
EOF
  ...
  # set address family if requested
  if [[ -n "${ADDRESS_FAMILY:-}" ]]; then
    if grep -q 'option[[:space:]]\+transport.address-family' "$vol"; then
      sed -ri "s#(option[[:space:]]+transport\.address-family[[:space:]]+).*#\\1${ADDRESS_FAMILY}#g" "$vol"
    else
      # insert right before end-volume
      sed -i "/end-volume/i \ \ \ \ option transport.address-family ${ADDRESS_FAMILY}" "$vol"
    fi
  fi
  # ensure max-port exists (to match EXPOSE)
  if grep -q 'option[[:space:]]\+max-port' "$vol"; then
    sed -ri "s#(option[[:space:]]+max-port[[:space:]]+).*#\\160999#g" "$vol"
  else
    sed -i "/end-volume/i \ \ \ \ option max-port 60999" "$vol"
  fi
}
...
start_glusterd(){
  ensure_glusterd_vol
  mkdir -p "$BRICK_PATH"; if [[ -n "$BRICK_PATHS" ]]; then to_arr "$BRICK_PATHS"; for bp in "${_ARR[@]}"; do mkdir -p "$bp"; done; fi
